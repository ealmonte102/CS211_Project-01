...
void RosterSystem::mainMenu( ) {
	...
	if (choice == 1 && loginStatus != SUPERVISOR) { choice = 5; }
...
void RosterSystem::displayAdminMenu ( ) const {
	for (int i = 0; i < numOfMenuOpts; ++i) {
		cout << i + 1 << ") " << menuOpts[i] << "\n";
	}
}

void RosterSystem::displayUserMenu( ) const {
	int i = 0;
	if (loginStatus == SUPERVISOR) { i = numOfMenuOpts - 1; }
